...
.NOTES
    ...
    Modified by Christian Kielhorn
...
.RELEASENOTES
    ...
    1.1 2024-07-02 downloading Bootstrapper directly from Internet/CDN
...
.CHANGELOG
    ...
    1.1.2407.1 - modified version to download Teams Bootstrapper directly from MS CDN
#>
...
[string]$CorpDataPath           = "C:\ProgramData\CKCTools"  #Path for the logfiles / temp. download

[string]$TeamsBootStrapper_URL  = "https://statics.teams.cdn.office.net/production-teamsprovision/lkg/teamsbootstrapper.exe"
[string]$TeamsBootStrapper_Path = "$($CorpDataPath)\TeamsBootStrapper"

[string]$NewTeamsInstaller      = "$($TeamsBootStrapper_Path)\teamsbootstrapper.exe" #Path to the new Teams installer
...
#Delete probably existing Teams 2.0 Bootstrapper File from pre-defined location
try     { Get-Item $TeamsBootStrapper_Path | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue
            Write-Verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),info,Success to remove $($TeamsBootStrapper_Path)"}
catch   { write-verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),error,Failed to remove $($TeamsBootStrapper_Path)"}

#Create pre-defined location for Downloading Bootstrapping File
If(!(Test-Path -Path $TeamsBootStrapper_Path)) {
    New-Item -ItemType Directory -Path $TeamsBootStrapper_Path
    Write-Verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),info,Success to (re-)create $($TeamsBootStrapper_Path)"}
Else {
    Write-Verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),warning,Recreation failed - Folder exists $($TeamsBootStrapper_Path)"
}

# Download current Teams 2.0 Bootstrapper directly from Internet
try     { Invoke-WebRequest -Uri $TeamsBootStrapper_URL -MaximumRedirection 10 -UseBasicParsing -OutFile $NewTeamsInstaller
            Write-Verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),info,Success to download to $($TeamsBootStrapper_Path)"
            Get-ChildItem -Path $TeamsBootStrapper_Path -Recurse | Unblock-File
            Write-Verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),info,Success to Unblock-File in $($TeamsBootStrapper_Path)" }
catch   { write-verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),error,Failed to download to $($TeamsBootStrapper_Path)"}

# Install New Teams
$process = Start-Process -FilePath $NewTeamsInstaller -ArgumentList $NewTeamsInstallerArgs -Wait -PassThru -ErrorAction STOP
if ($process.ExitCode -eq 0){
    write-verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),info,Success to Install $($NewTeamsInstaller)"
    $Global:EventId=10
    $Global:EventType="Information"
} else {
    write-verbose -verbose "$(Get-Date -Format 'yyyy-MM-dd'),$(Get-Date -format 'HH:mm:ss'),error,Failed to Install $($NewTeamsInstaller) with exit code $($process.ExitCode)"
    $Global:EventId=11
